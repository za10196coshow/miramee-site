...
初回だけ、黒い画面とブラウザを使った認証があります。

1. BATに `XXXX-XXXX` 形式の一時コードが表示されます。
2. コードはクリップボードへ自動的にコピーされます。
3. `Press Enter to open github.com in your browser...` と表示されたら、Enterを押します。
4. ブラウザが開いたらGitHubへログインします。
5. コード入力欄が出たら、`Ctrl+V`でコードを貼り付けます。
6. `Continue`を押し、続いて`Authorize GitHub`を押します。
7. `Success`と表示されたら認証完了です。黒い画面に戻ると処理が自動で続きます。

このコードは短時間だけ使える一時的な認証コードです。GitHubのパスワードをBATへ入力したり、ファイルへ保存したりする必要はありません。
...
同じ名前のリポジトリがすでにGitHubにある場合も、BATがGitHub側の履歴を先に取得してから最新版を追加します。再実行時に既存の履歴を強制削除することはありません。
